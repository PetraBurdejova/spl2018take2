devtools::install_github("lborke/yamldebugger", force = T)
...
yaml.debugger.run = function(qnames, init.obj) {
  ...
  for (i in 1:n) {
    ...
    for (qlet in q_files) {
      ...
      f_ext = tolower(tools::file_ext(qlet))
      ...
      if( tolower(qlet) == "metainfo.txt" ){
        ...
        result = try( yaml_meta <- yaml::yaml.load_file(q_full_path), silent = FALSE )
        ...
        if (class(result) == "try-error") {
          ...
          QDescription	= gsub("\n", " ", QDescription)
          desc_stat		= stringi::stri_stats_latex(QDescription)
    ...
    fnames = tolower(tools::file_path_sans_ext(names(t_vec)))
